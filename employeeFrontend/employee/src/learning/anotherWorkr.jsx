...
const WorkReport = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("/api/work-reports") // Replace with the correct API URL for fetching work reports
      .then((response) => {
  ...
  const handleSaveAsDraft = () => {
    ...
    if (formData.id) {
      ...
      axios
        .put(`/api/work-reports/${formData.id}`, reportData)
        .then((response) => {
          setReports((prev) =>
            prev.map((r) =>
  ...
  const handleSubmit = (e) => {
    ...
    if (
      window.confirm(
        "Are you sure you want to submit this report for review? If you submit it then you cannot edit it ."
      )
    ) {
      if (formData.id) {
        axios
          .put(`/api/work-reports/${formData.id}`, formData) // Update the report
          .then((response) => {
            setReports((prev) =>
              prev.map((r) =>
                r.id === formData.id
        ...
        axios
          .post("/api/work-reports", formData) // Create a new report
          .then((response) => {
            setReports((prev) => [...prev, response.data]);
          })
          .catch((error) => console.error("Error saving work report:", error));
